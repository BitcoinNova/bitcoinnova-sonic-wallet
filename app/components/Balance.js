...
export default class Balance extends Component<Props, State> {
  ...
  switchCurrency = () => {
    ...
    if (displayCurrency === 'fiat') {
      ...
      session.modifyConfig('displayCurrency', 'BTN');
      eventEmitter.emit('modifyCurrency', 'BTN');
    }
    ReactTooltip.rebuild();
  };
  ...
  render() {
    ...
    if (session.wallet && displayCurrency === 'BTN') {
      balanceTooltip =
        `Unlocked: ${session.atomicToHuman(unlockedBalance, true)} ${
          il8n.BTN
        }<br>` +
        `Locked: ${session.atomicToHuman(lockedBalance, true)} ${il8n.BTN}`;
    } else if (
      session.wallet &&
