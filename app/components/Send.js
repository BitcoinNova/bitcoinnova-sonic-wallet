...
export default class Send extends Component<Props, State> {
  ...
  handleAmountChange = (event: any) => {
    ...
    const fee = displayCurrency === 'BTN' ? 0.01 : 0.01 * fiatPrice;

    const totalAmount = (
      parseFloat(enteredAmount) +
      fee +
      parseFloat(session.daemon.feeAmount / 1000000)
    ).toFixed(6);
    this.setState({
      enteredAmount,
      totalAmount
  ...
  handleTotalAmountChange = (event: any) => {
    ...
    const subtractFee =
      Number(totalAmount) * 10000 - parseInt(session.daemon.feeAmount, 10);

    const enteredAmount =
      subtractFee < 0
        ? ''
        : session.atomicToHuman(subtractFee, false).toFixed(6);
  ...
  confirmTransaction = (event: any) => {
    ...
    const sufficientUnlockedFunds =
      session.getUnlockedBalance() > Number(totalAmount) / 1000000;
  ...
  sendTransaction = async () => {
    ...
    const [hash, err] = await session.sendTransaction(
      ...
      displayCurrency === 'BTN'
        ...
        : (Number(enteredAmount) * 1000000) / fiatPrice,
      paymentID
    );
    if (!loopTest) {
      if (hash) {
  ...
  createTestTransaction = async () => {
    ...
    await this.setState({
      ...
      totalAmount: String((amount + 10) / 1000000),
      sendToAddress,
      paymentID
    });
    ...
    if (loopTest && !looping) {
      ...
      this.loopInterval = setInterval(this.createTestTransaction, 10);
    }
  ...
  sendAll = () => {
    ...
    const totalAmount =
      unlockedBalance - 10000 - parseInt(session.daemon.feeAmount, 10) <= 0
        ? 0
        : unlockedBalance;
    const enteredAmount =
      unlockedBalance - 10000 - parseInt(session.daemon.feeAmount, 10) <= 0
        ? 0
        : totalAmount - 10000 - parseInt(session.daemon.feeAmount, 10);
    this.setState({
      totalAmount:
        displayCurrency === 'BTN'
    ...
    const addressInput = (
      ...
      >
        <Creatable
          ...
          placeholder="Enter a Bitcoin Nova address or a contact name to send funds to ..."
