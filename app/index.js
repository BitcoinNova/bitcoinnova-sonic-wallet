...
remote.app.setAppUserModelId('wallet.sonic.bitcoinnova');

log.debug(`BitcoinNova Sonic Wallet started...`);
  ...
  const message = (
    <div>
      ...
      <p className={`subtitle ${textColor}`}>
        There&apos;s a new version of BitcoinNova Sonic Wallet available. Would you like to
        download it?
      </p>
    </div>
  );
...
ipcRenderer.on('handleSaveAs', () => {
  ...
  const options = {
    ...
    filters: [
      {
        name: 'BitcoinNova Sonic Wallet File (v0)',
        extensions: ['wallet']
      }
    ]
  };
...
eventEmitter.on('sendNotification', function sendNotification(amount) {
  ...
  if (notifications) {
    const notif = new window.Notification('Transaction Received!', {
      body: `${il8n.just_received} ${amount} ${il8n.BTN}`
    });
    if (notif) {
      log.debug(`Sent notification: You've just received ${amount} BTN.`);
    }
  }
});
...
function handleHelp() {
  remote.shell.openExternal('https://discord.gg/3XuP44z');
}
  ...
  const message = (
    <div>
      ...
      <p className={`subtitle ${textColor}`}>
        ...
        BitcoinNovad in the Terminal tab.
      </p>
    </div>
  );
